...
function composer() {
    ...
    class Composer extends Compiler {
        ...
        // customized managed action, that is wrapper action into a composer.try() with default error handler
        managed_action(name, options = {}) {
            if (arguments.length > 2) throw new ComposerError('Too many arguments')
            name = parseActionName(name) // throws ComposerError if name is not valid
            let exec
            if (Array.isArray(options.sequence)) { // native sequence
            ...
            const body_composition = { type: 'action', name }
            if (exec) composition.action = { exec }

            var handler_name = parseActionName("compose-test/ServiceNowIncidentHandler")
            const handler_composition = {type: 'action', name: handler_name}

            return new Composition({type: 'try', body: new Composition(body_composition), handler: new Composition(handler_composition)})
        }
